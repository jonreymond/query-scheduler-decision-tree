...
//    val numPartitions_l = List(8)
    ...
    val q_q_list = (0 until q_list.length).flatMap(i =>
              (i until q_list.length).map(j => q_list(i) + "_" + q_list(j))).toList


    val writers = q_q_list.map(x =>
...
//      rdd_list.foreach(x => println(x.count()))
      ...
      (0 until q_list.length).foreach{
        i => (i until q_list.length).foreach{ j =>
          val results = numPartitions_l.map{numPartitions =>
            ...
            val measurements = (1 to num_measurements).map(_ =>
            {
              val start = System.nanoTime()
              val output = parallel(queryHandler.get(q_list(i))(), queryHandler.get(q_list(j))())
            ...
            println(q_list(i)+ " " + q_list(j) + "   num_core : " + num_core + " num_partitions : " + numPartitions + " ")
            Thread.sleep(10000)
            println(avg_timing/1000 + " [s]")
            println(measurements.map(t => t._2/1000).toList)
            avg_timing
        }

          writers(q_list(i) + "_" + q_list(j)).writeRow(List(num_core) ++ results)
      }
      }
      sc.stop()
    }
